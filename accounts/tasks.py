...
async def fetch_yandex(session, url, search_string, yandex_region):
...
async def fetch_google(session, url, search_string, google_region):
...
async def process_row(session, row, google_region, yandex_region):
    yandex_task = fetch_yandex(session, row['url'], row['search_string'], yandex_region)
    google_task = fetch_google(session, row['url'], row['search_string'], google_region)
    ...
    yandex_result['yandex_region'] = yandex_region  # Добавление региона Yandex в результат
    google_result['url'] = row['url']
    google_result['search_string'] = row['search_string']
    google_result['google_region'] = google_region  # Добавление региона Google в результат
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            for _, row in df.iterrows():
                task = asyncio.ensure_future(process_row(session, row, google_region, yandex_region))
                tasks.append(task)
            ...
            for result in results:
                ...
                combined_increase_freq = {k: max(yandex_result.get('увеличить частотность', {}).get(k, 0), google_result.get('увеличить частотность', {}).get(k, 0)) for k in set(yandex_result.get('увеличить частотность', {}).keys()).union(google_result.get('увеличить частотность', {}).keys())}
                combined_decrease_freq = {k: min(yandex_result.get('уменьшить частотность', {}).get(k, 0), google_result.get('уменьшить частотность', {}).get(k, 0)) for k in set(yandex_result.get('уменьшить частотность', {}).keys()).union(google_result.get('уменьшить частотность', {}).keys())}
